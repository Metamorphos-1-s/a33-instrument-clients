...
public sealed class PersistenceEvidenceGateTests
{
    ...
    public void HistoricalFirmwareManifestIsRejected(ushort firmware)
    {
        Assert.Throws<InvalidDataException>(() => PersistenceBaselineContract.Validate(
            Baseline().Manifest with { FirmwareVersion = firmware }));
    }

    [Fact] public void HistoricalBaselineRootIsNotTheCurrentContract()
    {
        Assert.NotEqual("Results/pc_stage2b_050b_baseline/persistence_baseline_manifest.json",
            PersistenceBaselineContract.RelativeManifestPath);
        Assert.NotEqual("a33-stage2b-fw050b-brightness3-20260909",
            PersistenceBaselineContract.BaselineId);
    }

    [Fact] public void PersistenceRunnerUsesOnlyTheCentralCurrentEvidenceRoot()
    {
        Assert.Equal("Results/pc_stage2c_0510_hw", Stage2BDeviceContract.PersistenceEvidenceRoot);
    ...
    private static async Task<CompleteEvidence> CreateCompleteEvidenceAsync(string root)
    {
        ...
        var mailbox=new MailboxSnapshot(0,0,0,0,new ushort[12]);var identity=new DeviceIdentity(0x0510,2,0x0104,1);var at=preflight.Now.AddMinutes(2);
    ...
    private static StrictPreflightReport Report()
    {
        ...
        return new StrictPreflightReport(true, [], new(0x0510, 2, 0x0104, 1),
            ...
            new ConfigStorePreflightEvidence(A33.Instrument.Protocol.WordOrder.HighWordFirst,[0,1,0,0,3,0,3],[3,1,0,3,0],Store()), baseline.Manifest.BaselineId, baseline.Manifest.ActiveArraySha256, baseline.ManifestSha256,
    ...
    private static ConfigStoreSnapshot Store() => new(0, 0, ConfigStoreState.Idle, ConfigStoreState.Idle, false, 3, 3, 3, 1, 3);
    ...
    private sealed class PersistenceTransport(IEnumerable<ushort> baseline):IModbusTransport
    {
        ...
        private bool dirty;private uint currentRevision=3;private uint savedRevision=3;private ushort activeSlot=1;private uint activeSequence=3;
        ...
        private ushort[] Read(ushort address,ushort count)
        {
            ...
            {
                0=>Realtime(),14=>[0x0104,0x0510],0x0030=>Diagnostics(),0x004C=>Mailbox(),
